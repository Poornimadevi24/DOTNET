...
SELECT job, AVG(sal) AS avg_salary, COUNT(*) AS total_employees FROM EMP GROUP BY job;
...
---9. Get the names and salaries of all the analysts earning more than 1200 who are based in department  Sort the answer by ascending order of name. 
SELECT ename, sal FROM EMP WHERE job = 'ANALYST' AND sal > 1200 AND deptno = 20 ORDER BY ename ASC;

---10. For each department, list its name and number together with the total salary paid to employees in that department. 
SELECT d.deptno, d.dname, SUM(e.sal) AS total_salary FROM DEPT d JOIN EMP e ON d.deptno = e.deptno GROUP BY d.deptno, d.dname;
...
SELECT ename, sal, sal * 12 AS yearly_salary FROM EMP WHERE ename = 'SMITH';

---14. List the name and salary for all employees whose salary is not in the range of 1500 and 2850. 
SELECT ename, sal FROM EMP WHERE sal NOT BETWEEN 1500 AND 2850;

---15. Find all managers who have more than 2 employees reporting to them
SELECT mgr_id, COUNT(*) AS total_subordinates FROM EMP WHERE mgr_id IS NOT NULL GROUP BY mgr_id HAVING COUNT(*) > 2;